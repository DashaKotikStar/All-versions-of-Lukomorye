# Lukomorie.py
import pygame
import sys
import math
from Wheel_Aprons import DiceWheel
from player_logic import PlayerLogic

pygame.init()

SCREEN_WIDTH = 1920
SCREEN_HEIGHT = 1030
screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
pygame.display.set_caption("Лукоморье 1")

WHITE = (255, 255, 255)
BLACK = (0, 0, 0)
RED = (255, 0, 0)
GREEN = (0, 255, 0)
BLUE = (0, 0, 255)
YELLOW = (255, 255, 0)
GRAY = (200, 200, 200)
PURPLE = (100, 0, 200)
light_blue = (0, 153, 125)

font_small = pygame.font.SysFont('Arial', 24)
font_medium = pygame.font.SysFont('Arial', 36)
font_large = pygame.font.SysFont('Arial', 48)
font_huge = pygame.font.SysFont('Arial', 72)

try:
    board_image = pygame.image.load("lukomorie_board1.jpg")
    board_image = pygame.transform.rotate(board_image, -90)
    board_image = pygame.transform.scale(board_image, (SCREEN_WIDTH - 300, SCREEN_HEIGHT))
except FileNotFoundError:
    print("Файл 'lukomorie_board1.jpg' не найден.")
    board_image = pygame.Surface((SCREEN_WIDTH - 300, SCREEN_HEIGHT))
    board_image.fill(GREEN)
    pygame.draw.rect(board_image, BLACK, (0, 0, SCREEN_WIDTH - 300, SCREEN_HEIGHT), 2)
    text = font_medium.render("Фон", True, WHITE)
    board_image.blit(text, (10, 10))

CELL_POSITIONS = {
    1: (100, 150),
    2: (250, 300),
    3: (400, 400),
    4: (150, 500),
    5: (300, 600),
    6: (450, 500),
    7: (550, 300),
    8: (600, 400),
    9: (700, 500),
    10: (800, 300),
    11: (900, 400),
    12: (1000, 300),
    13: (1100, 500),
}

player = PlayerLogic("Игрок 1")
wheel = DiceWheel(SCREEN_WIDTH - 160, SCREEN_HEIGHT // 2, 150, font_large, font_huge)

# Статусы
dice_result_handled = False
extra_roll_handled = False  # для доп. бросков

INVENTORY_RECT = pygame.Rect(SCREEN_WIDTH - 300, 100, 280, 100)
BON_DISPLAY_RECT = pygame.Rect(SCREEN_WIDTH - 300, 250, 280, 50)

clock = pygame.time.Clock()
running = True

while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False

        if event.type == pygame.MOUSEBUTTONDOWN and not player.in_mini_game:
            mouse_x, mouse_y = event.pos
            dist_sq = (mouse_x - wheel.center_x) ** 2 + (mouse_y - wheel.center_y) ** 2
            if dist_sq <= wheel.radius ** 2:
                if not wheel.is_spinning():
                    if not player.skip_turn:
                        # Основной ход: всегда крутим
                        wheel.spin()
                        dice_result_handled = False
                        extra_roll_handled = False
                    elif player.needs_extra_roll:
                        # Доп. бросок на 2/9/10
                        wheel.spin()
                        extra_roll_handled = False

    wheel.update()

    # Основной бросок → перемещение
    if not wheel.is_spinning() and not dice_result_handled and not player.in_mini_game and not player.skip_turn:
        dice_roll = wheel.get_dice_result()
        if dice_roll != 0:
            print(f"[Ход] Основной бросок: {dice_roll}")
            player.move_forward(dice_roll)
            player.handle_cell_after_move()
            dice_result_handled = True

            # Победа?
            if player.position == 13 and player.inventory[0] == "Волшебный клубок":
                print("🎉 Победа!")

    # Доп. бросок → обработка особой клетки
    if not wheel.is_spinning() and not extra_roll_handled and not player.in_mini_game and player.needs_extra_roll:
        dice_roll = wheel.get_dice_result()
        if dice_roll != 0:
            print(f"[Доп. бросок] на клетке {player.position}: {dice_roll}")
            player.handle_extra_roll(dice_roll)
            extra_roll_handled = True

    # Экран
    screen.fill(WHITE)
    screen.blit(board_image, (0, 0))

    # Фишка
    if player.position in CELL_POSITIONS:
        x, y = CELL_POSITIONS[player.position]
        pygame.draw.circle(screen, RED, (x, y), 15)
        label = font_small.render(str(player.position), True, WHITE)
        screen.blit(label, (x - 5, y - 10))

    wheel.draw(screen)

    # UI
    pygame.draw.rect(screen, GRAY, INVENTORY_RECT)
    screen.blit(font_small.render("Инвентарь:", True, BLACK), (INVENTORY_RECT.x + 5, INVENTORY_RECT.y + 5))
    for i, item in enumerate(player.inventory):
        txt = font_small.render(item or "Пусто", True, BLACK)
        screen.blit(txt, (INVENTORY_RECT.x + 5, INVENTORY_RECT.y + 30 + i * 20))

    pygame.draw.rect(screen, YELLOW, BON_DISPLAY_RECT)
    screen.blit(font_medium.render(f"Боны: {player.bon}", True, BLACK), (BON_DISPLAY_RECT.x + 10, BON_DISPLAY_RECT.y + 10))

    # Мини-игра
    if player.in_mini_game:
        screen.fill(BLACK)
        title = "Кот Учёный" if player.position == 3 else "Царевна Лягушка" if player.position == 5 else "Мини-игра"
        screen.blit(font_large.render(f"Мини-игра: {title}", True, WHITE), (SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT//2 - 50))
        screen.blit(font_small.render("Нажмите любую клавишу...", True, WHITE), (SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT//2 + 50))
        pygame.display.flip()
        waiting = True
        while waiting:
            for ev in pygame.event.get():
                if ev.type == pygame.QUIT:
                    pygame.quit()
                    sys.exit()
                if ev.type == pygame.KEYDOWN:
                    waiting = False
                    player.exit_mini_game()

    pygame.display.flip()
    clock.tick(60)

pygame.quit()
sys.exit()